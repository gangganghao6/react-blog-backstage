 ...
 return <Drawer title="自定义你的封面" placement="right" onClose={closeDrawer} visible={visible} size={'large'}>
  <Radio.Group onChange={selectPost} value={postOriginSrc}>
   <Space direction="vertical">
    {imgPathNames ? imgPathNames.data.data.concat(data.data.data.images).slice((page - 1) * 10, page * 10).map((item) => {
  ...
  <div style={{marginTop: '10px', textAlign: 'center'}}>
   <Pagination pageSize={10}
               total={imgPathNames ? imgPathNames.data.data.concat(data.data.data.images).length : data ? data.data.data.images.length : 0}
               onChange={setPage}/>
  </div>
